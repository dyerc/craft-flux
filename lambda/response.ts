...
export const handler = async (
  ...
  // Non-ascii characters in the URI arrive percent encoded.
  try {
  ...
  if (response.status === "403" || response.status === "404") {
    ...
    if (transform) {
      return await fetchSource(transform, config)
        ...
        .then((buffer) => {
          ...
          if (buffer.length > 512 * 1024) {
            response.headers["Location"] = [
              { key: "Location", value: encodeURI(removeRootPrefix(request.uri, config)) },
            ];
