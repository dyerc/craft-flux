...
async function handle(
  ...
  let parts = url.split("?");
  const event = createCloudFrontResponseEvent(
    Object.assign(
      {},
      {
        uri: parts[0],
...
describe("response", () => {
  ...
  test("uses source filename header if provided", async () => {
    ...
    const result = await handle(
      "/testAssets/data/folder/_500xAUTO_fit_center-center_80/image.webp?mode=fit&w=500",
      {
        status: "403",
        requestHeaders: {
  ...
  test("handles S3 source", async () => {
    ...
    const result = await handle(
      "/s3Assets/folder/_500xAUTO_stretch_center-center_100/image.jpg?mode=fit&w=500",
      { status: "403" },
      {
        verifyQuery: false,
        sources: [
  ...
  test("responds with png image", async () => {
    ...
    const result = await handle(
      "/testAssets/_600xAUTO_fit_center-center/image.png?mode=fit&w=600&f=png",
      {
        status: "403",
        requestHeaders: {
          "x-flux-source-filename": [
