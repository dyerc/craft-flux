...
async function handle(url: string, options = {}, config = {}): Promise<CloudFrontRequest> {
  const context = createCloudfrontContext();

  let parts = url.split("?");
  const event = createCloudFrontRequestEvent(Object.assign({}, {
    uri: parts[0],
    querystring: parts[1]
  }, options));
...
describe('request', () => {
  test('parses mode', async () => {
    const result = await handle("/images/image.jpg?mode=fit&w=1920&h=1080", {}, { verifyQuery: false });
  ...
  test('ignores request without a dimension', async () => {
    const result = await handle("/images/image.jpg?mode=fit&q=70", {}, { verifyQuery: false });
    expect(result.uri).toEqual("/images/image.jpg");
  });

  test('processes request with only one dimension', async () => {
    const result = await handle("/images/image.jpg?mode=fit&h=920&q=70", {}, { verifyQuery: false });
    expect(result.uri).toEqual("/images/_AUTOx920_fit_center-center_70/image.jpg");
  });

  test('processes deeply nested file', async () => {
    const result = await handle("/images/1/2/3/4/5/6/image.jpg?mode=fit&h=920&q=70", {}, { verifyQuery: false });
    expect(result.uri).toEqual("/images/1/2/3/4/5/6/_AUTOx920_fit_center-center_70/image.jpg");
  });

  test('processes request with crop position', async () => {
    const result = await handle("/images/image.jpg?mode=fit&h=920&q=70&pos=bottom-center", {}, { verifyQuery: false });
  ...
  test('processes request with webp image format', async () => {
    const result = await handle("/images/image.jpg?mode=fit&w=1920&h=1080&f=webp", {}, { verifyQuery: false });
    expect(result.uri).toEqual("/images/_1920x1080_fit_center-center_80/image.webp");
  ...
  test('processes request with png image format', async () => {
    const result = await handle("/images/image.jpg?mode=fit&w=600&f=png", {
      requestHeaders: {
        accept: [{
          key: "accept",
          value: "image/webp"
  ...
  test('validates verification token', async () => {
    const url = "images/image.jpg?mode=crop&position=center-center&w=1920&h=1080";
    ...
    expect(result.uri).toEqual("/images/_1920x1080_crop_center-center_80/image.jpg");
  });
  ...
  test('responds with a webp if accepted', async () => {
    const result = await handle("/images/image.jpg?mode=fit&w=1920&h=1080", {
      requestHeaders: {
        accept: [{
          key: "accept",
          value: "image/webp"
