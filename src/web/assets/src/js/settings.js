$(document).ready(function() {
  ...
  const $cfRefreshDistributionsSpinner = $cfRefreshDistributionsBtn.parent().next().children();
    ...
    Craft.sendActionRequest('POST', 'flux/aws/load-bucket-data', {data})
      .then(({data}) => {
        ...
        for (let i = 0; i < data.buckets.length; i++) {
          ...
          $s3BucketSelect.append('<option value="' + data.buckets[i].bucket + '" data-region="' + data.buckets[i].region + '">' + data.buckets[i].bucket + '</option>');
        }
  ...
  $cfRefreshDistributionsBtn.click(function() {
    ...
    Craft.sendActionRequest('POST', 'flux/aws/load-distributions-data', {data})
      .then(({data}) => {
        ...
        for (let i = 0; i < data.distributions.length; i++) {
          ...
          $cfDistributionSelect.append('<option value="' + data.distributions[i].id + '" data-domain="' + data.distributions[i].domain + '">' + data.distributions[i].id + '</option>');
        }
